		...
		')
			...
			foreach ($memberb as $m) {
				if (empty($m['email_name'])) {
					...
					$this->update('UPDATE ' . PREFIX . 'mailbox_member SET email_name = ' . $this->strval($m['name'] . '@' . DEFAULT_EMAIL_HOST) . ' WHERE mailbox_id = ' . (int)$m['id'] . ' AND foodsaver_id = ' . (int)$this->func->fsId());
				}
				$mboxes[] = array(
					'id' => $m['id'],
					'name' => $m['name'],
