			...
			if (isset($_POST['name'])) {
				if ($mailbox = $this->model->filterName($_POST['name'])) {
					if ($this->model->addMailbox($mailbox, 1)) {
						$this->func->info($this->translationHelper->s('mailbox_add_success'));
						$this->routeHelper->go('/?page=mailbox&a=manage');
					} else {
						$this->func->error($this->translationHelper->s('mailbox_already_exists'));
					}
				}
			}
		...
		if ($this->session->isOrgaTeam()) {
			if (isset($_POST['mbid'])) {
				...
				if ($this->model->updateMember($_POST['mbid'], $g_data[$index])) {
					$this->func->info($this->translationHelper->s('edit_success'));
					$this->routeHelper->go('/?page=mailbox&a=manage');
				}
			}
