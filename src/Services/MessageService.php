...
class MessageService
{
	...
	public function __construct(EmailHelper $emailHelper, FoodsaverGateway $foodsaverGateway, Mem $mem, TranslationHelper $translationHelper, Db $legacyDb, MessageModel $messageModel)
	{
	...
	private function sendMailNotification(int $senderId, int $recipientId, string $message, string $notificationTemplate = null): void
	{
		...
		if ((int)$info > 0) {
			...
			if (!$this->mem->userIsActive($recipientId)) {
				if (!isset($_SESSION['lastMailMessage'][$recipientId]) || (time() - $_SESSION['lastMailMessage'][$recipientId]) > 600) {
					...
					$this->emailHelper->tplMail($notificationTemplate, $foodsaver['email'], array(
