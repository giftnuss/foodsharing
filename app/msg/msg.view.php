		...
		if(!empty($conversations))
		{
			foreach ($conversations as $c)
			{
				...
				if(!empty($c['member']))
				{
					...
					$list .= '<li id="convlist-'.$c['id'].'"><a href="#" onclick="'.$click.'('.$c['id'].');info.$allWrapper.hide();return false;"><span class="pics">'.$pics.'</span><span class="names">'.$title.'</span><span class="msg">'.$c['last_message'].'</span><span class="time">'.niceDate($c['last_ts']).'</span><span class="clear"></span></a></li>';
				}
			}
		}
		else
