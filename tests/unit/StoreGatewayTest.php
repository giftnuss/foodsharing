...
class StoreGatewayTest extends \Codeception\Test\Unit
{
	...
	public function testUpdateExpiredBellsRemovesBellIfNoUnconfirmedFetchesAreInTheFuture()
	{
		...
		$this->tester->updateInDatabase(
			...
			['identifier' => 'store-urequest-' . $store['id']]
		); // outdate bell notification

		$this->gateway->updateExpiredBells();

		$this->tester->dontSeeInDatabase('fs_bell', ['identifier' => 'store-urequest-' . $store['id']]);
	}
	...
	public function testStoreBellsAreGeneratedCorrectly()
	{
		...
		$this->tester->seeNumRecords(1, 'fs_bell', ['identifier' => 'store-urequest-' . $store['id']]);

		$bellVars = $this->tester->grabFromDatabase('fs_bell', 'vars', ['identifier' => 'store-urequest-' . $store['id']]);
		$vars = unserialize($bellVars);
		$this->assertEquals(2, $vars['count']);

		$bellDate = $this->tester->grabFromDatabase('fs_bell', 'time', ['identifier' => 'store-urequest-' . $store['id']]);
