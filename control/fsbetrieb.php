...
else if(isset($_GET['id']))
{
	...
	');
		...
		{
			$menu[] = array('name'=>'Nachricht ans Team','href'=> '?page=message&a=neu&list='.$betrieb['id']);
			$menu[] = array('name'=>'Nachricht an Springer','href'=> '?page=message&a=neu&slist='.$betrieb['id']);
		}
			...
			if (isMob())
			{
				$out .= '
					<li class="jumper fs-'.$fs['id'].'">
						<div class="ui-corner-all'.$class.'">
							<a title="#tt-tt-'.$fs['id'].'" href="#" onclick="'.$click.'return false;"><img class="ui-corner-all" src="'.img($fs['photo']).'" alt="'.$fs['vorname'].'" /></a>
							<span class="infos">
								<span class="item"><a title="#tt-tt-'.$fs['id'].'" href="#" onclick="'.$click.'return false;"><strong>'.$fs['name'].'</strong></a></span>
								'.$tel.'
							</span>
						</div>
						<span style="display:none" class="tt-'.$fs['id'].'">
...
function u_form_abhol_table($zeiten = false,$option = array())
{
	...
	$out = '
		<table class="timetable">
			...
			<tfoot>
			    <tr>
					<td colspan="3"><span id="nft-add">add</span></td>		
				</tr>
			</tfoot>
			<tbody>';
	$dows = range(1,6);
		...
		foreach ($zeiten as $z)
		{
			...
			<tr class="'.$odd.'">
			    <td class="ui-padding">
				  ...
				  <td class="ui-padding"><input class="fetchercount" style="width:25px;" type="text" name="nft-count[]" value="'.$z['fetcher'].'" /><span style="float:right;" class="nft-remove">'.s('remove').'</span></td>
			    </tr>';
		}
	}
	$out .= '</tbody></table>';
	...
	$out .= '<table id="nft-hidden-row" style="display:none;">
			...
			<tr>
			    <td class="ui-padding">
				  ...
				  <td class="ui-padding"><input class="fetchercount" style="width:25px;" type="text" name="nft-count[]" value="2" /><span style="float:right;" class="nft-remove">'.s('remove').'</span></td>
			    </tr>
				</tbody>
			</table>';
...
function u_form_abhol_tableOld($zeiten = false,$option = array())
{
	$dow = range(1,7);
	$days = getDow();
	...
	addJs('
		$(".fetchercount").bind("keyup",function(){
			...
			{
				val = 2;
				pulseError("Du kannst maximal 5 Menschen das Abholen ermöglichen.<br />In der Regel sollten aber <strong>nicht mehr als 2 Leute</strong> zu einem Laden gehen.",{
					sticky:true
				});
			}
			else if(val > 2)
			{
				pulseInfo("Du hast mehr als 2 Leute zum abholen angegeben.<br />In der Regel sollten <strong>nicht mehr als 2 Leute</strong> zu einem Betrieb gehen. Zu viele Abholer führten schon oft zum Ende einer Koorperation. <br />Zur Not geht einer von euch mit Auto oder Anhänger vor und ihr trefft euch außer Reichweite vom Betrieb.",{
					sticky:true
				});
			}
			this.value = val;
		...
		$("table.timetable td label").click(function(){
			...
			if(cb[0].checked)
			{
				...
				$("#fetcheranz-"+cb.attr("value")).css("visibility","visible");
			}
			else
			{
				$("#timepick-"+cb.attr("value")).css("visibility","hidden");
				$("#fetcheranz-"+cb.attr("value")).css("visibility","hidden");
			}
		})
	');
	...
	$out = '
		<table class="timetable">
			...
			<thead>
				<tr class="milestone">
					<th class="ui-padding">'.s('day').'</th><th class="ui-padding">'.s('time').'</th><th class="ui-padding">'.s('fetcher_count').'</th>
				</tr>
			</thead>
			<tbody>';
	...
	foreach ($days as $dow => $d)
	{
		...
		if($zeiten !== false && isset($zeiten[$dow]))
		{
			$hidden = 'visible';
			$chk = ' checked="checked"';
			$value = $zeiten[$dow]['time'];
			$fetcher_v = $zeiten[$dow]['fetcher'];
		}
		
		$out .= '
			<tr class="'.$odd.'">
				<td class="ui-padding"><label><input type="checkbox" name="dow[]" value="'.$dow.'"'.$chk.' /> '.$d.'</label></td><td><div id="timepick-'.$dow.'" style="visibility:'.$hidden.';">'.v_form_time('day'.$dow,$value).'</div></td><td><div id="fetcheranz-'.$dow.'" style="visibility:'.$hidden.';"><input class="fetchercount" style="width:25px;" type="text" name="fetcher'.$dow.'" value="'.$fetcher_v.'" /> '.s('fetcher').'</div></td>
			</tr>';
	}
